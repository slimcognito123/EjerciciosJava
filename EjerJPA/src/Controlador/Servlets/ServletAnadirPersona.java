...
public class ServletAnadirPersona extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (expresion.matches(expresion.pattern(), telefono)) {
            ...
            Contacto contacto = new Contacto(-1, nombre, apellido, telefono, fechaCompleta);
            AgendaController controller = new AgendaController();
            controller.anadirPersonaOnline(contacto, String.valueOf(request.getSession(false).getAttribute("user")));
