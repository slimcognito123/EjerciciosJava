...
public class EscritorBD {
    ...
    public void modificarPersonaOnline(Persona persona, String user) {
        String query = "update agenda set nombre=?, apellidos=?, telefono=?, FNaci=? where dni=? and user=?";
        try{
            ...
            sentenciaPreparada.setString(5,persona.getDni());
            ...
            sentenciaPreparada.setString(4, diaFechaNacimiento+"-"+ mesFechaNacimiento+"-"+ anoFechaNacimiento);
            sentenciaPreparada.setString(6,user);
    ...
    public boolean eliminarPersonaOnline(String dni, Object user) {
        try {
            Statement peticion = conectorBD.getConnection().createStatement();
            String query = "delete from agenda where dni='"+dni+"' and user='"+user+"';";
            peticion.executeUpdate(query);
        } catch (SQLException e) {
